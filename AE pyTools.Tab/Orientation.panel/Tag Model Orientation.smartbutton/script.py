...
if not hasattr(config, 'tag_model_orientation'):
    config.tag_model_orientation = False  # Default to OFF


# Function to toggle the state and update the icon
def toggle_tag_model_orientation():
    # Toggle the state
    config.tag_model_orientation = not config.tag_model_orientation

    # Update the button icon
    script.toggle_icon(config.tag_model_orientation)

    # Save the updated config
    script.save_config()

    # Display feedback
    state = "ON" if config.tag_model_orientation else "OFF"
    balloon_msg = "Tag Model Orientation toggled to: {}".format(state)
...
def __selfinit__(script_cmp, ui_button_cmp, __rvt__):
    ...
    icon_path = on_icon if config.tag_model_orientation else off_icon
    ui_button_cmp.set_icon(icon_path)

# Main script logic
if __name__ == '__main__':
    toggle_tag_model_orientation()
